...
class HierarchicalCNNTransformer(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        # ── Tabular embedders ────────────────────────────────────────────────
        # Future embedder takes num_lags lag tensors plus calendar features.
    ...
    def adapt_inputs(
        ...
        # 4. Two lags, both within the 168-h history window:
        #    - 168-h lag (first 24 of history): same hour, one week prior
        #    - 24-h lag  (last 24 of history):  same hour, one day prior
    ...
    def forward(
        ...
        future_weather:        torch.Tensor,   # (B, 24, G, G, C)
        future_calendar:       torch.Tensor,   # (B, 24, F)
        future_demand_lag_168: torch.Tensor,   # (B, 24, Z)   168-h prior
        future_demand_lag_24:  torch.Tensor,   # (B, 24, Z)   24-h prior
    ) -> torch.Tensor:                         # (B, 24, Z)   raw MWh (denormalised)
        B = hist_weather.size(0)
        T = self.S + 24
        ...
        hist_tab   = torch.cat([hist_demand_norm, hist_calendar],   dim=-1)  # (B, S,  Z+F)
        future_tab = torch.cat([lag168_norm, lag24_norm, future_calendar], dim=-1)  # (B, 24, 2Z+F)
